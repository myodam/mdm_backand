-- =============================================================
-- 테스트 전용 V2 (H2 호환)
-- COMMENT 제거, CLOB 사용, ADD COLUMN IF NOT EXISTS
-- =============================================================

ALTER TABLE story_scenes
    ADD COLUMN IF NOT EXISTS success_message VARCHAR(255) NULL,
    ADD COLUMN IF NOT EXISTS fail_messages   CLOB         NULL,
    ADD COLUMN IF NOT EXISTS before_message  VARCHAR(255) NULL;

-- scene_000: 튜토리얼
UPDATE story_scenes
SET before_message  = '몸을 카메라 앞에 세우고 준비해보세요!',
    success_message = '마법의 도서관에 오신 것을 환영합니다. 책을 펼쳐 이야기 속 주인공이 되어 볼까요?',
    fail_messages   = '{"BOOK_NOT_TURNED": "왼손으로 책장을 오른쪽으로 넘겨주세요.","HANDS_TOO_FAR": "책은 더 가까이서 잡아 주세요.","LOW_SCORE": "조금 더 크게 동작해볼까요?","USER_NOT_DETECTED": "카메라 앞에 서서 다시 시도해주세요.","HAND_NOT_VISIBLE": "손이 화면 안에 보이도록 해주세요."}'
...
SET before_message  = '은혜를 입은 제비가 흥부에게 보은을 하려나봐요! 손을 내밀어봐요',
    ...
    fail_messages   = '{"HANDS_TOO_FAR":"두 손을 조금 더 가까이 모아주세요.","HAND_NOT_RAISED":"두 손을 어깨보다 높게 들어주세요.","LOW_SCORE":"조금 더 크게 동작해볼까요?","USER_NOT_DETECTED":"카메라 앞에 서서 다시 시도해주세요.","HAND_NOT_VISIBLE":"손이 화면 안에 보이도록 해주세요."}'
...
SET before_message  = '제비가 준 씨앗이 커다란 박으로 자랐어요! 어서 잘라보아요',
    ...
    fail_messages   = '{"HANDS_TOO_FAR":"우리 친구 카메라 앞에 서서 다시 해볼까요?.","ARMS_NOT_WIDE":"양팔을 더 크게 벌려주세요.","MOVEMENT_TOO_SMALL":"손을 조금 더 크게 움직여주세요.","LOW_SCORE":"조금 더 크게 동작해볼까요?","USER_NOT_DETECTED":"카메라 앞에 서서 다시 시도해주세요.","HAND_NOT_VISIBLE":"손이 화면 안에 보이도록 해주세요."}'
